"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.GeographyDumpController = void 0;
const common_1 = require("@nestjs/common");
const geography_dump_service_1 = require("./geography-dump.service");
const get_country_dto_1 = require("./dtos/get-country.dto");
const get_state_dto_1 = require("./dtos/get-state.dto");
const get_city_dto_1 = require("./dtos/get-city.dto");
const standard_api_headers_constant_1 = require("../../../shared/constants/standard-api-headers.constant");
const swagger_1 = require("@nestjs/swagger");
const custom_header_decorator_1 = require("../../../shared/decorators/common/custom-header.decorator");
const standard_api_responses_constant_1 = require("../../../shared/constants/standard-api-responses.constant");
let GeographyDumpController = class GeographyDumpController {
    geographyDumpService;
    constructor(geographyDumpService) {
        this.geographyDumpService = geographyDumpService;
    }
    async getCountryList(getCountryDto, headers) {
        try {
            return await this.geographyDumpService.getCountryList(getCountryDto);
        }
        catch (error) {
            throw new common_1.HttpException(error.message, error.status || common_1.HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    async getStateList(getStateDto, headers) {
        try {
            return await this.geographyDumpService.getStateList(getStateDto);
        }
        catch (error) {
            throw new common_1.HttpException(error.message, error.status || common_1.HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    async getCityList(getCityDto, headers) {
        try {
            return await this.geographyDumpService.getCityList(getCityDto);
        }
        catch (error) {
            throw new common_1.HttpException(error.message, error.status || common_1.HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    async updateCityVectorAndCityNameNormalized(headers) {
        try {
            return await this.geographyDumpService.updateCityVectorAndCityNameNormalized();
        }
        catch (error) {
            throw new common_1.HttpException(error.message, error.status || common_1.HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
};
exports.GeographyDumpController = GeographyDumpController;
__decorate([
    (0, swagger_1.ApiOperation)({ summary: 'Get country list' }),
    (0, swagger_1.ApiResponse)(standard_api_responses_constant_1.SWG_SUCCESS_RESPONSE),
    (0, swagger_1.ApiResponse)(standard_api_responses_constant_1.SWG_NOT_FOUND_RESPONSE),
    (0, swagger_1.ApiResponse)(standard_api_responses_constant_1.SWG_BAD_REQUEST_RESPONSE),
    (0, swagger_1.ApiResponse)(standard_api_responses_constant_1.SWG_UNPROCESSABLE_RESPONSE),
    (0, swagger_1.ApiResponse)(standard_api_responses_constant_1.SWG_INTERNAL_SERVER_ERROR_RESPONSE),
    (0, common_1.Get)('countries'),
    (0, common_1.HttpCode)(common_1.HttpStatus.OK),
    __param(0, (0, common_1.Query)()),
    __param(1, (0, common_1.Headers)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [get_country_dto_1.GetCountryDto, Object]),
    __metadata("design:returntype", Promise)
], GeographyDumpController.prototype, "getCountryList", null);
__decorate([
    (0, swagger_1.ApiOperation)({ summary: 'Get state list' }),
    (0, swagger_1.ApiResponse)(standard_api_responses_constant_1.SWG_SUCCESS_RESPONSE),
    (0, swagger_1.ApiResponse)(standard_api_responses_constant_1.SWG_NOT_FOUND_RESPONSE),
    (0, swagger_1.ApiResponse)(standard_api_responses_constant_1.SWG_BAD_REQUEST_RESPONSE),
    (0, swagger_1.ApiResponse)(standard_api_responses_constant_1.SWG_UNPROCESSABLE_RESPONSE),
    (0, swagger_1.ApiResponse)(standard_api_responses_constant_1.SWG_INTERNAL_SERVER_ERROR_RESPONSE),
    (0, common_1.Get)('states'),
    (0, common_1.HttpCode)(common_1.HttpStatus.OK),
    __param(0, (0, common_1.Query)()),
    __param(1, (0, common_1.Headers)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [get_state_dto_1.GetStateDto, Object]),
    __metadata("design:returntype", Promise)
], GeographyDumpController.prototype, "getStateList", null);
__decorate([
    (0, swagger_1.ApiOperation)({ summary: 'Get city list' }),
    (0, swagger_1.ApiResponse)(standard_api_responses_constant_1.SWG_SUCCESS_RESPONSE),
    (0, swagger_1.ApiResponse)(standard_api_responses_constant_1.SWG_NOT_FOUND_RESPONSE),
    (0, swagger_1.ApiResponse)(standard_api_responses_constant_1.SWG_BAD_REQUEST_RESPONSE),
    (0, swagger_1.ApiResponse)(standard_api_responses_constant_1.SWG_UNPROCESSABLE_RESPONSE),
    (0, swagger_1.ApiResponse)(standard_api_responses_constant_1.SWG_INTERNAL_SERVER_ERROR_RESPONSE),
    (0, common_1.Get)('cities'),
    (0, common_1.HttpCode)(common_1.HttpStatus.OK),
    __param(0, (0, common_1.Query)()),
    __param(1, (0, common_1.Headers)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [get_city_dto_1.GetCityDto, Object]),
    __metadata("design:returntype", Promise)
], GeographyDumpController.prototype, "getCityList", null);
__decorate([
    (0, swagger_1.ApiOperation)({ summary: 'Update city vector and city name normalized' }),
    (0, swagger_1.ApiResponse)(standard_api_responses_constant_1.SWG_SUCCESS_RESPONSE),
    (0, swagger_1.ApiResponse)(standard_api_responses_constant_1.SWG_NOT_FOUND_RESPONSE),
    (0, swagger_1.ApiResponse)(standard_api_responses_constant_1.SWG_BAD_REQUEST_RESPONSE),
    (0, swagger_1.ApiResponse)(standard_api_responses_constant_1.SWG_UNPROCESSABLE_RESPONSE),
    (0, swagger_1.ApiResponse)(standard_api_responses_constant_1.SWG_INTERNAL_SERVER_ERROR_RESPONSE),
    (0, common_1.Get)('update-city-vector'),
    (0, common_1.HttpCode)(common_1.HttpStatus.OK),
    __param(0, (0, common_1.Headers)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], GeographyDumpController.prototype, "updateCityVectorAndCityNameNormalized", null);
exports.GeographyDumpController = GeographyDumpController = __decorate([
    (0, swagger_1.ApiTags)('Geography Dump'),
    (0, swagger_1.ApiHeaders)([
        standard_api_headers_constant_1.SWG_HEADER_CURRENCY_PREFERENCE,
        standard_api_headers_constant_1.SWG_HEADER_LANGUAGE_PREFERENCE_MANDATE,
        standard_api_headers_constant_1.SWG_HEADER_IP_MANDATE,
        standard_api_headers_constant_1.SWG_HEADER_API_VERSION_MANDATE,
        standard_api_headers_constant_1.SWG_HEADER_CLUB_ID_MANDATE,
        standard_api_headers_constant_1.SWG_HEADER_DEVICE_INFORMATION_MANDATE,
    ]),
    (0, custom_header_decorator_1.RequiredHeaders)([
        standard_api_headers_constant_1.DEC_HEADER_LANGUAGE_PREFERENCE_MANDATE,
        standard_api_headers_constant_1.DEC_HEADER_IP_ADDRESS_MANDATE,
        standard_api_headers_constant_1.DEC_HEADER_API_VERSION_MANDATE,
        standard_api_headers_constant_1.DEC_HEADER_CLUB_ID_MANDATE,
        standard_api_headers_constant_1.DEC_HEADER_CURRENCY_PREFERENCE_MANDATE,
        standard_api_headers_constant_1.DEC_HEADER_DEVICE_INFORMATION_MANDATE,
    ]),
    (0, common_1.Controller)('dump/geography'),
    __metadata("design:paramtypes", [geography_dump_service_1.GeographyDumpService])
], GeographyDumpController);
//# sourceMappingURL=geography-dump.controller.js.map