...
export class TboBookService {
    ...
    async createSingleBook(reqParams): Promise<{ ticketingResult: any; requestBodyTicketing?: any }[]> {
        const {
            ...
            logCollector,
        }: { bookRequest: any; index?: number; handleAuthenticationFailure: (...args: any[]) => any; logCollector?: SupplierLogCollector } = reqParams;
    ...
    async ticketingCall(reqParams) {
        const {
            ...
            supplierResult = null,
            logCollector,
        }: { bookRequest: any; pnr: string; bookingId: string; bookTraceId: string; index: number; supplierResult?: any; logCollector?: SupplierLogCollector } = reqParams;
        const { providerCred } = bookRequest;
    ...
    async createMultipleBook(reqParams) {
        const { bookRequest, handleAuthenticationFailure, logCollector }: { bookRequest: any; handleAuthenticationFailure: (...args: any[]) => any; logCollector?: SupplierLogCollector } = reqParams;
        const { bookReq }: { bookReq: BookDto } = bookRequest;
