    ...
    settingUpPrices(searchReq, passengerFareArr, fareBreakDown, preferredCurrency, FareType) {
        ...
        fareBreakDown.forEach((element) => {
            ...
            if (element.PassengerType === 1) {
                /* Adult */
                fareDetail.perPersonAdultFare = Generic.currencyConversion(element?.BaseFare / passengerCount, element.Currency, preferredCurrency) || 0;
                fareDetail.adultFare = fareDetail.perPersonAdultFare * adultCount;

                /* Tax calculation */
                adultTaxPP = element?.Tax ? Generic.currencyConversion(element?.Tax / passengerCount, element.Currency, preferredCurrency) : 0;

                /* Service fee calculation */
                adultSFeePP = element?.ServiceFee ? Generic.currencyConversion(element?.ServiceFee / passengerCount, element.Currency, preferredCurrency) : 0;

                /* Other charges calculation */
                adultOChargePP = element?.OtherCharges ? Generic.currencyConversion(element?.OtherCharges / passengerCount, element.Currency, preferredCurrency) : 0;
            } else if (element.PassengerType === 2) {
                /* Child */
                fareDetail.perPersonChildFare = childCount > 0 ? Generic.currencyConversion(element?.BaseFare / passengerCount, element.Currency, preferredCurrency) : 0;
                fareDetail.childFare = fareDetail.perPersonChildFare * childCount;

                /* Tax calculation */
                childTaxPP = element?.Tax ? Generic.currencyConversion(element?.Tax / passengerCount, element.Currency, preferredCurrency) : 0;

                /* Service fee calculation */
                childSFeePP = element?.ServiceFee ? Generic.currencyConversion(element?.ServiceFee / passengerCount, element.Currency, preferredCurrency) : 0;

                /* Other charges calculation */
                childOChargePP = element?.OtherCharges ? Generic.currencyConversion(element?.OtherCharges / passengerCount, element.Currency, preferredCurrency) : 0;
            } else if (element.PassengerType === 3) {
                /* Infant */
                fareDetail.perPersonInfantFare = infantCount > 0 ? Generic.currencyConversion(element?.BaseFare / passengerCount, element.Currency, preferredCurrency) : 0;
                fareDetail.infantFare = fareDetail.perPersonInfantFare * infantCount;

                /* Tax calculation */
                infantTaxPP = element?.Tax ? Generic.currencyConversion(element?.Tax / passengerCount, element.Currency, preferredCurrency) : 0;

                /* Service fee calculation */
                infantSFeePP = element?.ServiceFee ? Generic.currencyConversion(element?.ServiceFee / passengerCount, element.Currency, preferredCurrency) : 0;

                /* Other charges calculation */
                infantOChargePP = element?.OtherCharges ? Generic.currencyConversion(element?.OtherCharges / passengerCount, element.Currency, preferredCurrency) : 0;
            }
        });
